...
public class SwiftFlutterSensorsPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let channel = FlutterMethodChannel(name: "flutter_sensors_plus", binaryMessenger: registrar.messenger())
    ...
    private func startEventChannel(sensorId: Int, interval: Double)->Bool{
        ...
        switch sensorId {
        case AccelerometerStreamHandler.SENSOR_ID:
            let accelerometerEventChannel = FlutterEventChannel(name:"flutter_sensors_plus/\(AccelerometerStreamHandler.SENSOR_ID)", binaryMessenger: registrar.messenger())
            accelerometerEventChannel.setStreamHandler(accelerometerStreamHandler)
            break
        case GyroscopeStreamHandler.SENSOR_ID:
            let gyroscopeEventChannel = FlutterEventChannel(name:"flutter_sensors_plus/\(GyroscopeStreamHandler.SENSOR_ID)", binaryMessenger: registrar.messenger())
            gyroscopeEventChannel.setStreamHandler(gyroscopeStreamHandler)
            break
        case MagneticFieldStreamHandler.SENSOR_ID:
            let magneticFieldEventChannel = FlutterEventChannel(name:"flutter_sensors_plus/\(MagneticFieldStreamHandler.SENSOR_ID)", binaryMessenger: registrar.messenger())
            magneticFieldEventChannel.setStreamHandler(magneticFieldStreamHandler)
            break
        case LinearAccelerationStreamHandler.SENSOR_ID:
            let linearAccelerationEventChannel = FlutterEventChannel(name:"flutter_sensors_plus/\(LinearAccelerationStreamHandler.SENSOR_ID)", binaryMessenger: registrar.messenger())
            linearAccelerationEventChannel.setStreamHandler(linearAccelerationStreamHandler)
            break
        case StepDetectorStreamHandler.SENSOR_ID:
            let stepDetectorEventChannel = FlutterEventChannel(name:"flutter_sensors_plus/\(StepDetectorStreamHandler.SENSOR_ID)", binaryMessenger: registrar.messenger())
            stepDetectorEventChannel.setStreamHandler(stepDetectorStreamHandler)
            break
        case HeadingStreamHandler.SENSOR_ID:
            let headingEventChannel = FlutterEventChannel(name:"flutter_sensors_plus/\(HeadingStreamHandler.SENSOR_ID)", binaryMessenger: registrar.messenger())
